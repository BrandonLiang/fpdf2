...
class MultiCellTest(unittest.TestCase):
    ...
    def test_multi_cell_ln_0(self):
        ...
        assert_pdf_equal(self, doc, "test_multi_cell_ln_0.pdf")

    def test_multi_cell_ln_1(self):
        """
        ...
        assert_pdf_equal(self, doc, "test_multi_cell_ln_1.pdf")

        doc = fpdf.FPDF()
        doc.add_page()
        doc.set_font("helvetica", size=TEXT_SIZE)
        doc.multi_cell(w=100, h=LINE_HEIGHT, border=1, txt="Lorem ipsum")
        doc.ln()
        doc.multi_cell(w=100, h=LINE_HEIGHT, border=1, txt="Ut nostrud irure")
        assert_pdf_equal(self, doc, "test_multi_cell_ln_1.pdf")

    def test_multi_cell_ln_3(self):
        ...
        for row in data:
            for datum in row:
                pdf.multi_cell(col_width, line_height, str(datum), border=1, ln=3)
            pdf.ln(line_height)
        assert_pdf_equal(self, pdf, "test_multi_cell_ln_3.pdf", generate=True)
